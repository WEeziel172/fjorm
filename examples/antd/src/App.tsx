...
const btnActive = (color: string) => ({
  ...btnBase, border: `2px solid ${color}`, background: '#e6f4ff', color: color,
})
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => setView('build')}
            style={view === 'build' ? btnActive('#1677ff') : btnBase}>Build</button>
          <button onClick={handlePreview}
            style={view === 'preview' ? btnActive('#1677ff') : btnBase}>Preview</button>
        </div>
      </nav>

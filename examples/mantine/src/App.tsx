...
const btnActive = (color: string) => ({
  ...btnBase, border: `2px solid ${color}`, background: '#e7f5ff', color: color,
})
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => setView('build')}
            style={view === 'build' ? btnActive('#228be6') : btnBase}>Build</button>
          <button onClick={handlePreview}
            style={view === 'preview' ? btnActive('#228be6') : btnBase}>Preview</button>
        </div>
      </nav>
